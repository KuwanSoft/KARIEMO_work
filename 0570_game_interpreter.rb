  ...
  def command_111
    ...
    when 4  # アクター
      ...
      if actor != nil
        ...
        when 2  # スキル
          result = (actor.skill_learn?($data_magics[@params[3]]))
        when 3  # 武器
...
#~       result = $game_party.has_item?($data_armors[@params[1]], @params[2])
  ...
  def command_123
    if @original_event_id > 0
      key = [@map_id, @original_event_id, @params[0], $game_player.x, $game_player.y] # プレイヤーの座標をイベントの座標として扱う
